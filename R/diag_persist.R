...
#'
#'@param atBtxt A data frame. Total biomass of all groups over time, read in from
#'Atlantis ...BioInd.txt output using \code{atlantisom::load_bioind}.
#'@param fgs.names Character vector. A vector of species names in which to test for persistence.
#'(Default = NULL). For default all species in atBtxt
#'@param threshold Numeric scalar. Proportion of initial biomass for which for which all species
#'are measured against. Default = 0, all species need to be non zero.
...
#'}
#'
diag_persist <- function(atBtxt, fgs.names=NULL, threshold = 0, plot=TRUE){
  ...
  if (is.null(fgs.names)) { # select all species
    fgs.names <- unique(atBtxt$species)
  }

  # For each species calculate average biomass over each year.
  # Remember there are multiple time steps per year
  #
  crash <- atBtxt %>%
    dplyr::filter(species %in% fgs.names) %>%
    dplyr::mutate(yr = ceiling(time/365)) %>%
    dplyr::filter(yr > 0) %>%
    dplyr::group_by(species, yr) %>%
    dplyr::summarise(meanB = mean(atoutput)) %>%
    dplyr::filter(meanB == 0)
  ...
  if(plot){
    ...
    plotB <- ggplot2::ggplot() +
      ggplot2::geom_line(data=atBtxt %>% dplyr::filter(species %in% fgs.names),
